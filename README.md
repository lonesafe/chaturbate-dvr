...
--output-dir value          Directory to move completed recordings to
--per-model-folder          Create a subdirectory per model inside --output-dir
--segment-workers value     Maximum parallel downloads per track (default: 6)
--pending-seconds value     Maximum unmatched A/V timeline buffer (default: 60)
--max-pending-mb value      Maximum unmatched A/V memory per track (default: 512)
--min-free-disk-mb value    Stop below this free disk space (default: 512)
--sync-seconds value        Maximum interval between fsync calls (default: 3)
--sync-fragments value      Maximum fragments between fsync calls (default: 10)
--max-text-mb value         Maximum API/playlist response size (default: 4)
--max-segment-mb value      Maximum init/media segment size (default: 64)
--http-timeout-seconds value    API/playlist timeout (default: 30)
--segment-timeout-seconds value Media segment timeout (default: 120)
--help, -h                  show help
...
_Note: In Web UI mode, recording defaults and A/V timeline tolerance can be changed and persisted from Settings. A/V timeline tolerance defaults to 1000 ms and is managed only from Settings._
